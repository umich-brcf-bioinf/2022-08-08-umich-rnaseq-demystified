...
Alternatively, since `pca_plot` is a `ggplot`, we can also use `ggsave()`.
...
>
> It turns out that `pca_plot` object is a `ggplot`. You can see this with `class(pca_plot)`. This means we can use what we learned at the end of the Computational Foundations Workshop to modify this plot as we might see necessary. Try doing the following to the `pca_plot`:
>
...
**Checkpoint**: *If generated and saved the `pca_plot` PCA plot, please indicate with the green 'check' button. Otherwise, please use  use the red 'x' button in your zoom reaction panel to have this step repeated.*
...
>
> [Link to exercise](https://umich-brcf-bioinf.github.io/2022-03-14-umich-rnaseq-demystified/html/Module09a_breakout.html)
