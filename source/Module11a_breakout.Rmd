...
4. Optionally, how could we use some of the `tidyverse` functions we've encountered previously to rename the `external_gene_name` column to `symbol` and to move it into the second column position? Hint: Because of the order of the packages we may have loaded, consider using `dplyr::select()` instead of just the `select()` function. We can discuss why this is the case together.
